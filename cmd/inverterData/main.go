...
func init() {
	...
	inverterStringData = flag.Bool("string", false, "Inverter string data (default false)")
	flag.StringVar(&host, "host", "localhost", "host to connect (default localhost)")
	flag.StringVar(&port, "port", "8899", "port to connect (default 8899)")
...
func main() {
	...
	if *inverterStringData {
		value = append(value, sendAndProcess(register_start2, register_end2, con)...)
	}
	value = append(value, sendAndProcess(registerhw_start, registerhw_end, con)...)
